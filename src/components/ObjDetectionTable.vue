		...
		<div class="operationTable" style="box-shadow: 2px 2px 2px 2PX #d6d6d6;background-color: #fcfcfc;">
			...
			<el-tabs v-model="activeName" type="card" :stretch="true">
					...
					<div v-show="!isFirstStep">
						<el-form :model="form" size="mini">
							...
							<el-form-item>
								...
								<el-button style="position: absolute;right:8%; width: 80px;" @click="DetectionObj"
									size="mini">检测
								</el-button>
							</el-form-item>
				...
				<el-tab-pane label="批量处理" name="second">
					<BatchProcessingTableA operationAPI="odHistorys"></BatchProcessingTableA>
...
<script>
	...
	export default {
		...
		computed: {
			...
			processTitle() {
				if (this.$route.query.processType === "playground") {
					return "操场检测"
				}
				if (this.$route.query.processType === "overpass") {
					return "立交桥检测"
				}
				if (this.$route.query.processType === "aircraft") {
					return "飞机检测"
				}
			},
			projectList() {
		...
		methods: {
			...
			DetectionObj() {
				...
				if (!that.$store.state.flag0) {
					axios.post(
						`${this.$store.state.serverURL}/odHistorys?projectId=${this.form.projectTitle}&title=${this.form.operationId}`,
						formData, {
							headers: {
						...
						axios.post(
							`${that.$store.state.serverURL}/odHistorys/process/${that.$route.query.processType}?flag=${flag}` +
					...
					var jsonTmp = JSON.stringify(tmp)
					axios.post(
						`${that.$store.state.serverURL}/odHistorys/process/${that.$route.query.processType}?flag=${that.$store.state.flag0}` +
