			...
			<el-tabs v-model="activeName" type="card" :stretch="true">
				<el-tab-pane label="单次处理" name="first">
					<div v-show="isFirstStep">
						<div style="display: inline-block;float: left;">
							<SelectImageButton class="SIB" btnName="选择时像1"></SelectImageButton>
						</div>
						<div style="display: inline-block;float: left;">
							<SelectImageButton class="SIB" btnName="选择时像2" :disabled="true"></SelectImageButton>
						</div>
						<div style="display: inline-block;float: left;margin-left: 2%;">
							<SelectFiles />
						</div>

						<div style="display: inline-block;float: left;margin-left: 14%;margin-bottom: 3%;">
							<el-button size='mini' @click="isFirstStep=false"
								:disabled="!$store.state.oldTimeImageURL||!$store.state.newTimeImageURL">下一步</el-button>
						</div>
					...
					<div v-show="!isFirstStep">
						<el-form :model="form" size="mini">
							...
							<el-form-item label="所属项目" label-width="80px" filterable placeholder="请选择">
							...
							<el-form-item>
								...
								<el-button style="position: absolute;right:8%; width: 80px;" @click="ChangeObj"
									size="mini">
									检测
								</el-button>
							</el-form-item>
				...
				<el-tab-pane label="批量处理" name="second">
					<BatchProcessingTableB></BatchProcessingTableB>
				</el-tab-pane>
			</el-tabs>
		</div>
	</div>
...
<script>
	...
	import FilesListB from './FilesListB.vue'
	import BatchProcessingTableB from './BatchProcessingTableB.vue'
	...
	export default {
		...
		methods: {
			...
			resetRect() {
				...
				this.$store.state.drawnRectParams.width=this.$store.state.imgWidth1
				this.$store.state.drawnRectParams.height=this.$store.state.imgWidth1
				this.$store.state.clickCancelRectTimes += 1
			},
			ChangeObj() {
				if (this.$store.state.isImgLoading){
				...
				formData.append('file1', this.$store.state.oldTimeFile)
				formData.append('file2', this.$store.state.newTimeFile)
				this.$store.state.isImgLoading = true
				this.$store.state.loadingMsg = "上传图片中..."

				if (!that.$store.state.flag1 || !that.$store.state.flag2) {
					axios.post(
						`${this.$store.state.serverURL}/cdHistorys?projectId=${this.form.projectTitle}&title=${this.form.operationId}`,
						formData, {
							headers: {
						...
						var data1 = res.data.data.sourceImg1
						var data2 = res.data.data.sourceImg2
						var historyId = res.data.data.id
						data1 = data1.split('/')
						data2 = data2.split('/')
						var flag1 = data1[data1.length - 1]
						var flag2 = data2[data2.length - 1]
						that.$store.state.flag1 = flag1
						that.$store.state.flag2 = flag2
						...
						axios.post(
							`${that.$store.state.serverURL}/cdHistorys/process?flag1=${that.$store.state.flag1}&flag2=${that.$store.state.flag2}` +
			...
			ChangeObj2() {
				if (!this.form.projectId || !this.form.operationId || this.form.threshold <= 0) {
					this.$message({
						showClose: true,
						message: '存在选项为空！',
						type: 'error',
				...
				var resultData = {
					left: this.$store.state.drawnRectParams.left,
					top: this.$store.state.drawnRectParams.top,
					width: this.$store.state.drawnRectParams.width,
					height: this.$store.state.drawnRectParams.height
				}
				this.$store.commit('OBJEXTRACT')
				this.$store.state.resultImageURL.push(resultData)
