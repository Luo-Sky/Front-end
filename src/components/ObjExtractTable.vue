		...
		<div class="operationTable" style="box-shadow: 2px 2px 2px 2PX #d6d6d6;background-color: #fcfcfc;">
			...
			<el-tabs v-model="activeName" type="card" :stretch="true">
				<el-tab-pane label="单次处理" name="first">
					<div v-show="isFirstStep">
						...
						<div style="margin-left: 30%;">
							<el-button size='mini' style="margin-left: 35%;" @click="isFirstStep=false"
					...
					<div v-show="!isFirstStep">
						<el-form :model="form" size="mini">
							...
							<el-form-item>
								...
								<el-button style="position: absolute;right:8%; width: 80px;" @click="ExtractObj"
									size="mini">提取
								</el-button>
							</el-form-item>
				...
				<el-tab-pane label="批量处理" name="second">
					<BatchProcessingTableA operationAPI="oeHistorys"></BatchProcessingTableA>
		...
		methods: {
			...
			ExtractObj() {
				...
				if (!that.$store.state.flag0) {
					axios.post(
						`${this.$store.state.serverURL}/oeHistorys?projectId=${this.form.projectTitle}&title=${this.form.operationId}`,
						formData, {
							headers: {
							...
							onUploadProgress: function(
								...
								if (progressEvent.lengthComputable) {
									...
									that.$store.state.upLoadProgress = Math.floor(progressEvent.loaded /
										progressEvent.total *
										99) //实时获取最新下载进度
						...
						axios.post(
							`${that.$store.state.serverURL}/oeHistorys/process/${that.$route.query.processType}?flag=${flag}` +
					...
					axios.post(
						`${that.$store.state.serverURL}/oeHistorys/process/${that.$route.query.processType}?flag=${that.$store.state.flag0}` +
